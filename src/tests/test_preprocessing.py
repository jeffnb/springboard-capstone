...
from src.text_preprocessing import remove_link, remove_discord_mention, remove_punc, preprocess_message


@pytest.mark.parametrize("token,expected", [("https://foo.com", None), ("httJustkidding", "httJustkidding")])
def test_remove_link(token, expected):
    assert remove_link(token) == expected


@pytest.mark.parametrize("token,expected", [("<@!1232123432432432423>", None), ("@FOOBAR", "@FOOBAR")])
